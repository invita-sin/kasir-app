...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    if (!admin) {
      httpRequestsTotal.inc({ method: "GET", path: "/api/stock-out", status: 403 });
    ...
    const result = await StockService.listStockOut({ page, limit, all });
...
export async function POST(req: NextRequest) {
  ...
  try {
    const admin = await requireAdmin(req);
    if (!admin) {
      httpRequestsTotal.inc({ method: "POST", path: "/api/stock-out", status: 403 });
      httpRequestDurationSeconds.observe({ method: "POST", path: "/api/stock-out" }, (Date.now() - start) / 1000);
      return NextResponse.json({ error: "Forbidden", code: "FORBIDDEN" }, { status: 403 });
    }

    const body = await parseJsonBody(req);
    const stockOut = await StockService.createStockOut(body);
