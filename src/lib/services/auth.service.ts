...
export const AuthService = {
  async login(body: unknown) {
    ...
    const [token, refreshToken] = await Promise.all([
      this.signToken({ userId: user.id, username: user.username, role: user.role, name: user.name }, "access"),
      this.signToken({ userId: user.id, username: user.username, role: user.role, name: user.name }, "refresh"),
    ]);

    return { token, refreshToken, username: user.username, name: user.name, role: user.role };
  },

  async signToken(payload: { userId: string; username: string; role: string; name: string }, type: "access" | "refresh" = "access"): Promise<string> {
  ...
  async deleteUser(id: string) {
    const existing = await prisma.user.findUnique({ where: { id } });
    if (!existing) throw new NotFoundError("User");
    if (existing.role === "ADMIN") throw new ValidationError("Tidak dapat menghapus admin");
